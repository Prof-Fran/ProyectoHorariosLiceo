...
window.Modulo_armado = (() => {
  ...
  let _docenteOcupacionExterna = {}; // Mapa celda "dia:hora" -> ocupado en otra institución
  let _docenteOcupacionInterna = {}; // Mapa celda "dia:hora" -> ocupado en otro grupo del liceo
  ...
  const URL_DISPONIBILIDAD_DOCENTE = (idDocente) => `${App.API_BASE}/disponibilidad/completa_docente/${idDocente}`;
  ...
  // ════════════════════════════════════════════════════════════
  //  Cargar disponibilidad del docente en el turno del grupo
  // ════════════════════════════════════════════════════════════

  async function _cargarDisponibilidadDocente() {
    if (!_grupoActual || !_docenteSeleccionado) return;

    try {
      const res = await fetch(URL_DISPONIBILIDAD_DOCENTE(_docenteSeleccionado.id));
  ...
  async function _seleccionarDocente(card) {
    ...
    // Cargar disponibilidad del docente en su turno para determinar celdas disponibles/ocupadas
    await _cargarDisponibilidadDocente();
    ...
    // Resaltar celdas donde se puede asignar (considerando disponibilidad)
  ...
  function _obtenerAsignacionesDocenteEnCelda(dia, hora, idDocente) {
    const asignaciones = _obtenerAsignacionesCelda(dia, hora);
    return asignaciones.filter(a => a.id_docente === idDocente);
  }
  ...
  async function _verificarConflictoExterno(idDocente, dia, hora) {
    try {
      ...
      if (_docenteOcupacionExterna && _docenteOcupacionExterna[clave]) {
